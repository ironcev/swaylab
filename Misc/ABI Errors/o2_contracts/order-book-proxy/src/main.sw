...
use contract_schema::proxys::{OrderbookProxy, OwnershipTransferredEvent};
...
impl OrderbookProxy for Contract {
    ...
    fn initialize_proxy() {
        ...
        require(
            asm(blob_id: INITIAL_TARGET, len) {
                bsiz len blob_id;
                len: u64
            } != 0,
            "Blob does not exist",
        ...
        storage::SRC14.target.write(Some(INITIAL_TARGET));
    ...
    fn set_owner(new_owner: Identity) {
        only_proxy_owner();
        let old_owner = _proxy_owner();
        _set_proxy_owner(State::Initialized(new_owner));
        OwnershipTransferredEvent::new(Some(old_owner.owner().unwrap()), new_owner)
            .log();
    }
}
