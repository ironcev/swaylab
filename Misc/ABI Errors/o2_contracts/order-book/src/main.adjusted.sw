...
impl OrderBook for Contract {
    ...
    fn create_order(order_args: OrderArgs) -> OrderId {
        ...
        if !(order_args
                .quantity != 0 && order_args
                .price != 0) {
            panic "OrderCreationError::InvalidOrderArgs";
        }
        ...
        if WHITE_LIST_CONTRACT.is_some() {
            ...
            if !(balance_of(
                ...
                panic "OrderCreationError::TraderNotWhiteListed";
            }
        }
        ...
        if BLACK_LIST_CONTRACT.is_some() {
            ...
            if !(balance_of(
                ...
                panic "OrderCreationError::TraderBlackListed";
            }
        }
        ...
        if !(order_args
                .price % PRICE_PRECISION == 0) {
            panic "OrderCreationError::PricePrecision";
        }
        ...
        if !ALLOW_FRACTIONAL_PRICE {
            ...
            // );
            if !( !quote_would_truncate(order_args.quantity, order_args.price)) {
                panic "OrderCreationError::FractionalPrice";
            }
        }
        ...
        match side {
            ...
            // ),
            Side::Buy => if !(msg_amount == quote_coins_from_quantity(order_args.quantity, order_args.price) && msg_amount >= MIN_ORDER) {
                panic "OrderCreationError::InvalidInputAmount";
            },
            // Side::Sell => require(
            ...
            //     OrderCreationError::InvalidInputAmount,
            // ),
            Side::Sell => if !(msg_amount == order_args
                    .quantity && quote_coins_from_quantity(order_args.quantity, order_args.price) >= MIN_ORDER) {
                panic "OrderCreationError::InvalidInputAmount";
            },
        }
        ...
        match storage::orderbook::v1.last_traded_price.try_read() {
            Some((last_traded_price, _)) => {
                if PRICE_WINDOW > 0 {
                    ...
                    // );
                    if !(price >= (last_price * (0x64u256 - price_window)) / 0x64u256 && price <= (last_price * (0x64u256 + price_window)) / 0x64u256) {
                        panic "OrderCreationError::PriceExceedsRange";
                    }
                }
            },
            None => {},
        ...
        let order_id = match execute_match_engine(
            ...
            OrderStatus::Unfilled((storage_order, matched_events)) => {
                ...
                match order_args.order_type {
                    ...
                    OrderType::FillOrKill => panic "OrderCreationError::OrderNotFilled",
                    ...
                    // ),
                    OrderType::PostOnly => if !(storage_order
                            .quantity == order_args
                            .quantity) {
                        panic "OrderCreationError::OrderPartiallyFilled";
                    },
                    _ => {}
                }
            ...
            OrderStatus::OutOfGas((order_id, match_events)) => {
                ...
                match order_args.order_type {
                    ...
                    OrderType::PostOnly => panic "OrderCreationError::OrderPartiallyFilled",
                    _ => {}
                }
...
impl OrderBookAdmin for Contract {
    ...
    fn initialize() {
        match INITIAL_OWNER.owner() {
            ...
            None => panic "OrderBookInitializationError::OwnerNotSet",
        }
        ...
        // );
        if !(BASE_ASSET != AssetId::zero() && QUOTE_ASSET != AssetId::zero()) {
            panic "OrderBookInitializationError::InvalidAsset";
        }
        // require(
        ...
        if !(BASE_DECIMALS >= QUOTE_DECIMALS) {
            panic "OrderBookInitializationError::InvalidDecimals";
        }
        // require(
        //     PRICE_WINDOW >= 0 && PRICE_WINDOW <= 100,
        //     OrderBookInitializationError::InvalidPriceWindow,
        // );
        if !(PRICE_WINDOW >= 0 && PRICE_WINDOW <= 100) {
            panic "OrderBookInitializationError::InvalidPriceWindow";
        }
        // require(MIN_ORDER > 0, OrderBookInitializationError::InvalidMinOrder);
        if !(MIN_ORDER > 0) {
            panic "OrderBookInitializationError::InvalidMinOrder";
        }

        match BASE_DECIMALS {
            0 => revert_with_log(OrderBookInitializationError::InvalidDecimals),
            0 => panic "OrderBookInitializationError::InvalidDecimals",
            _ => {
        ...
        match PRICE_PRECISION {
            ...
            0 => panic "OrderBookInitializationError::InvalidPricePrecision",
            _ => {
                ...
                // );
                if !(dec == 1) {
                    panic "OrderBookInitializationError::InvalidPricePrecision";
                }
            }
        }
    }
    ...
    fn cancel_blacklist_orders(orders: Vec<OrderId>) {
        ...
        if !(BLACK_LIST_CONTRACT.is_some()) {
            panic "OrderCancelError::NoBlacklist";
        }
    ...
    fn collect_fees(fee_recipient: Identity) {
        ...
        if !(fees != (0, 0)) {
            panic "FeeError::NoFeesAvailable";
        }
...
fn cancel_order(order_id: OrderId, cancel_type: CancelType) -> bool {
    ...
    match cancel_type {
        ...
        CancelType::Default => if !(order.trader_id == sender) {
            panic "OrderCancelError::NotOrderOwner";
        },
        ...
        // ),
        CancelType::Blacklist => if !balance_of(
            BLACK_LIST_CONTRACT
                .unwrap(),
            AssetId::new(BLACK_LIST_CONTRACT.unwrap(), order.trader_id.bits()),
        ) > 0 {
            panic "OrderCancelError::TraderNotBlacklisted";
        },
...
fn assert_heap_prices() {
        ...
        && highest_buy_price.is_some()
    {
        ...
        // );
        if !(lowest_sell_price.unwrap() > highest_buy_price.unwrap()) {
            panic "OrderCreationError::InvalidHeapPrices((lowest_sell_price.unwrap(), highest_buy_price.unwrap()))";
        }
    }
}
