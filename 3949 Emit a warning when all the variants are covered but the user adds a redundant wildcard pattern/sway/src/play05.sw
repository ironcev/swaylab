...
pub fn play() -> () {
    ...
    // BUG: Non-exhaustive match expression. Missing patterns `Struct { x: _, y: [1...MAX], ... }`
    // let _x = match s {
    //     Struct { x: true, y, z } => y + z.0,
    //     Struct { x: false, y, z } => y + z.0,
    ...
    // BUG: Non-exhaustive match expression. Missing patterns `(_, [1...MAX])`
    // let _x = match t {
    //     (true, _, s, n) => n + s.y,
    //     (false, _, s, n) => n + s.y,
    //     (b, e, s, 0) => {
    ...
    // BUG: Non-exhaustive match expression. Missing patterns `(_, Enum::A(_))`, `(_, Enum::E(_))`, `(_, Enum::D(_))`, `(_, Enum::C(_))`
    // let _x = match t5 {
    //     (true, _, s, n) => n + s.y,
    //     (false, _, s, n) => n + s.y,
